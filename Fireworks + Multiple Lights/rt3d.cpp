...
void setPointLightPos(const GLuint program, const GLfloat *lightPos) {
	int uniformIndex = glGetUniformLocation(program, "pointLights.position");
	glUniform3fv(uniformIndex, 1, lightPos);
}
...
//////-------Sets data for Point Light in shader------////////
void setPointLight(const GLuint program, const pointLightStruct light, GLint pointIndex) {
	// pass in light data to shader
	std::string locString = "pointLights[" + std::to_string(pointIndex) + "].ambient";
	int uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform3fv(uniformIndex, 1, light.ambient);
	locString = "pointLights[" + std::to_string(pointIndex) + "].diffuse";
	uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform3fv(uniformIndex, 1, light.diffuse);
	locString = "pointLights[" + std::to_string(pointIndex) + "].specular";
	uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform3fv(uniformIndex, 1, light.specular);

	locString = "pointLights[" + std::to_string(pointIndex) + "].position";
	uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform3fv(uniformIndex, 1, light.position);

	locString = "pointLights[" + std::to_string(pointIndex) + "].constant";
	uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform1f(uniformIndex, light.constant);
	locString = "pointLights[" + std::to_string(pointIndex) + "].linear";
	uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform1f(uniformIndex, light.linear);
	locString = "pointLights[" + std::to_string(pointIndex) + "].quadratic";
	uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform1f(uniformIndex, light.quadratic);

}

void setPointLightPosition(const GLuint program, const pointLightStruct light, GLint pointIndex)
{
	std::string locString = "pointLights[" + std::to_string(pointIndex) + "].position";
	int uniformIndex = glGetUniformLocation(program, locString.c_str());
	glUniform3fv(uniformIndex, 1, light.position);
}
/*
...
void setPointLight2(const GLuint program, const pointLightStruct light) {
	// pass in light data to shader
	int uniformIndex = glGetUniformLocation(program, "pointLights[1].ambient");
	glUniform3fv(uniformIndex, 1, light.ambient);
	uniformIndex = glGetUniformLocation(program, "pointLights[1].diffuse");
	glUniform3fv(uniformIndex, 1, light.diffuse);
	uniformIndex = glGetUniformLocation(program, "pointLights[1].specular");
	glUniform3fv(uniformIndex, 1, light.specular);

	uniformIndex = glGetUniformLocation(program, "pointLights[1].position");
	glUniform3fv(uniformIndex, 1, light.position);

	uniformIndex = glGetUniformLocation(program, "pointLights[1].constant");
	glUniform1f(uniformIndex, light.constant);
	uniformIndex = glGetUniformLocation(program, "pointLights[1].linear");
	glUniform1f(uniformIndex, light.linear);
	uniformIndex = glGetUniformLocation(program, "pointLights[1].quadratic");
	glUniform1f(uniformIndex, light.quadratic);
}
...
void setPointLight3(const GLuint program, const pointLightStruct light) {
	// pass in light data to shader
	int uniformIndex = glGetUniformLocation(program, "pointLights[2].ambient");
	glUniform3fv(uniformIndex, 1, light.ambient);
	uniformIndex = glGetUniformLocation(program, "pointLights[2].diffuse");
	glUniform3fv(uniformIndex, 1, light.diffuse);
	uniformIndex = glGetUniformLocation(program, "pointLights[2].specular");
	glUniform3fv(uniformIndex, 1, light.specular);

	uniformIndex = glGetUniformLocation(program, "pointLights[2].position");
	glUniform3fv(uniformIndex, 1, light.position);

	uniformIndex = glGetUniformLocation(program, "pointLights[2].constant");
	glUniform1f(uniformIndex, light.constant);
	uniformIndex = glGetUniformLocation(program, "pointLights[2].linear");
	glUniform1f(uniformIndex, light.linear);
	uniformIndex = glGetUniformLocation(program, "pointLights[2].quadratic");
	glUniform1f(uniformIndex, light.quadratic);
}*/

//////-----Sets data for toon shader lighting-----///////
...
///////------Add setDirectionalLight function-------///////
void setDirLight(const GLuint program, const dirLightStruct light) {
	// pass in light data to shader
	int uniformIndex = glGetUniformLocation(program, "dirLight.ambient");
	glUniform3fv(uniformIndex, 1, light.ambient);
	uniformIndex = glGetUniformLocation(program, "dirLight.diffuse");
	glUniform3fv(uniformIndex, 1, light.diffuse);
	uniformIndex = glGetUniformLocation(program, "dirLight.specular");
	glUniform3fv(uniformIndex, 1, light.specular);
	uniformIndex = glGetUniformLocation(program, "dirLight.direction");
	glUniform3fv(uniformIndex, 1, light.direction);
}
...
/////------sets material data for multiple lights shader------//////
void setLightMaterial(const GLuint program, const materialLightStruct material) {
	// pass in material data to shader 
	int uniformIndex = glGetUniformLocation(program, "material.diffuse");
	glUniform3fv(uniformIndex, 1, material.diffuse);
	uniformIndex = glGetUniformLocation(program, "material.specular");
	glUniform3fv(uniformIndex, 1, material.specular);
	uniformIndex = glGetUniformLocation(program, "material.ambient");
	glUniform3fv(uniformIndex, 1, material.ambient);
	uniformIndex = glGetUniformLocation(program, "material.shininess");
	glUniform1f(uniformIndex, material.shininess);
}
